...
// Test for the error path in handshakeInit_cmiStateServerWait when WriteMessageToWebsocketConnection fails
func (s *InitServerSuite) Test_ServerWait_WriteMessageError() {
	s.sut.setState(model.CmiStateServerWait, nil)
	...
	s.sut.handshakeInit_cmiStateServerWait(model.ShipInit)
